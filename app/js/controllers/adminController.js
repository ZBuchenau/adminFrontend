  ...
  vm.showPrintVersion = function() {
    ...
    for (var i = 0; i < mediaPlanService.mediaPlan.length; i++) {
      for (var j = 0; j < mediaPlanService.mediaPlan[i].length; j++) {
        // var newData = {
        //   "provider" : mediaPlanService.mediaPlan[i][j].provider_name,
        //   "spend" : mediaPlanService.mediaPlan[i][j].monthly_spend
        // };
        if(mediaPlanService.providerObj.hasOwnProperty(mediaPlanService.mediaPlan[i][j].provider_name)){
          mediaPlanService.providerObj[mediaPlanService.mediaPlan[i][j].provider_name] += parseInt(mediaPlanService.mediaPlan[i][j].monthly_spend);
        } else {
          mediaPlanService.providerObj[mediaPlanService.mediaPlan[i][j].provider_name] = parseInt(mediaPlanService.mediaPlan[i][j].monthly_spend);
        }
      }
